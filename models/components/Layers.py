...
from models.components.SubLayers import (
    ...
    CompositionalFFN, PairMultiHeadAttention,
)
...
class DecoderLayer(nn.Module):
    def __init__(self, opt: Dict[str, Any], is_last=False):
        ...
        elif opt.get('pair_multi_attention', False):
            inter_attention_class = PairMultiHeadAttention
        else:
    ...
    def get_infor(self):
        return self.inter_attention.get_infor()

    def get_cls(self, x):
        return  self.inter_attention.tag_head(x)
    ...
    def forward(self, 
        ...
        if hasattr(self, 'attr_attention') and self.attr_layer_pos == 'parallel':
            ...
            hidden_states, inter_attention_probs, context = self.inter_attention(
                hidden_states=hidden_states,
                encoder_hidden_states=encoder_hidden_states,
                attention_mask=attention_mask if hasattr(self.inter_attention, "tag_pred") else None,
                head_mask=head_mask,
                attend_to_video=True,
                **kwargs
            )
