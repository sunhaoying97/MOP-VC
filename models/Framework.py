...
class Seq2SeqBase(nn.Module):
    ...
    def encoding_phase(self, feats: List[torch.Tensor], **kwargs: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        inputs_for_decoder['feats'] = inputs_for_decoder['encoder_hidden_states']
        if self.predictor is not None and self.opt['task'] != "Class":
    ...
    def feedforward_step(self, batch: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        encoding_phase_outputs = self.encoding_phase(batch['feats'], batch=batch, **kwargs)
        inputs_for_decoder = encoding_phase_outputs
        inputs_for_decoder['video_ids'] = torch.LongTensor([int(batch['video_ids'][x][5:]) for x in range(len(batch['video_ids']))])
...
class TransformerSeq2Seq(Seq2SeqBase):
    def decoding_phase(self,
        ...
        # Model 1
        # if self.opt['task'] == 'Class' and kwargs.get('tslt', False):
        #     inputs_for_decoder['predict_data'] = self.predictor(input_ids=input_ids, tslt=kwargs.get('tslt', False))['tag_hidden_state']
        # Model 2
        if self.opt['task'] == 'Class':
            inputs_for_decoder['tag_hidden_state'] = self.predictor(input_ids=input_ids, embed=self.decoder.embedding(input_ids)[0])['tag_hidden_state']
