...
from models.components.SubLayers import PairMultiHeadAttention
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids, encoder_hidden_states=None, category=None, head_mask=None, return_input_embs=False, **kwargs):
        ...
        def build_mask(seq_len):
            """
            生成对角线为 False，上三角为 True，下三角为 False 的布尔矩阵
            """
            mask = torch.triu(torch.ones(seq_len, seq_len, dtype=torch.bool), diagonal=1)
            left_mask = torch.zeros((seq_len, kwargs['feats'].size(1)), dtype=torch.bool)# 对角线以上为 True
            mask = torch.cat((left_mask, mask), dim=1)
            return mask.unsqueeze(0).expand(attention_mask.size(0), -1, -1).to(attention_mask.device)

        if isinstance(self.layers[0].inter_attention, PairMultiHeadAttention):
            encoder_attention_mask = build_mask(input_ids.size(1))
        else:
        ...
        if self.opt['task'] == "Class":
            tag_infor = self.layers[0].get_infor()
